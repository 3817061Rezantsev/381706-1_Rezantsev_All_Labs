...
{
	TMonom X(3, ms1, 12);
	ASSERT_NO_THROW(X.SetCoeff(3.14));
	ASSERT_EQ(3.14, X.GetCoeff());
}
...
TEST(TPolynom, can_minus_equal_polynoms)
{
	...
	ASSERT_TRUE(*a == D);
	ASSERT_TRUE(a->GetCoeff() == -3);
	a = a->GetNext();
	ASSERT_TRUE(*a == B);
	ASSERT_TRUE(a->GetCoeff() == 2);
	a = a->GetNext();
	ASSERT_TRUE(a == 0);
}
...
TEST(TPolynom, assign_operator_polynoms_work)
{
	...
	TMonom* a = P.GetStart();
	ASSERT_TRUE(*a == D);
	ASSERT_EQ(3, a->GetCoeff());
	a = a->GetNext();
	ASSERT_TRUE(*a == B);
	ASSERT_EQ(2, a->GetCoeff());
	a = a->GetNext();
	ASSERT_TRUE(a == 0);
}
...
TEST(TPolynom, can_multiply_equal_polynoms)
{
	...
	TMonom AD = A * D;
	ASSERT_TRUE(*a == AD);
	ASSERT_EQ(a->GetCoeff(), 3);
	a = a->GetNext();
	ASSERT_TRUE(*a == AB);
	ASSERT_EQ(a->GetCoeff(), 2);
	a = a->GetNext();
	ASSERT_TRUE(a == NULL);
}
